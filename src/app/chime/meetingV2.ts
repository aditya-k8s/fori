...
var attendeeStart=0;
var streamTitle= "My video";
var AttendeeName= "Attendee Name-"+Date.now();
var streamLogo=  "<img src='https://fori.kindlebit.com/assets/images/logo.png' class='mark'>";//"https://fori.kindlebit.com/assets/images/logo.png";
...
export class DemoMeetingApp
  ...
  initParameters(): void {
    ...
    if (broadcaster!='true') {
      ...
      if (attendeeStart==0) {
        (document.getElementById('inputName') as HTMLInputElement).value =meeting+'+'+AttendeeName;
        console.log("Auto host submit>>>>>>>>>>>>>>>>>>>>>>>>>")
        this.autosubmit();
      }
    } else if (broadcaster=='true'){
      attendeeStart=1;
      (document.getElementById('inputMeeting') as HTMLInputElement).value = meeting;
      (document.getElementById('inputName') as HTMLInputElement).focus();
      if (attendeeStart==1) {
        (document.getElementById('inputName') as HTMLInputElement).value =AttendeeName;
        console.log("Auto non host submit>>>>>>>>>>>>>>>>>>>>>>>>>")
        this.autosubmit();
      }
    } else {
   ...
   autosubmit(): void {
      ...
      console.log('attendeeStart------>>>>>>>>>>>>>>>>>>>',attendeeStart);
      if (attendeeStart==1) {
  ...
  updateRoster(): void {
    ...
    document.getElementById('viwer').innerHTML ="Viewers ("+newRosterCount+")";
    if (attendeeStart==1) {
  ...
  setupDeviceLabelTrigger(): void {
    ...
    this.audioVideo.setDeviceLabelTrigger(
      async (): Promise<MediaStream> => {
        ...
        console.log('attendeeStart >>>>>>>>>>>validation>>>>>>>>>>>>>',attendeeStart)
        if (attendeeStart==1) {
          console.log("false -------- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"); 
            this.autoJoin();
  ...
  videoTileDidUpdate(tileState: VideoTileState): void {
    ...
    this.updateProperty(attendeeIdElement, 'innerText', streamTitle);//tileState.boundAttendeeId
    this.updateProperty(attendeeIdElementlogo, 'innerHTML', streamLogo);//tileState.boundAttendeeId
