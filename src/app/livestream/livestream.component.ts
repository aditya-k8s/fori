...
export class LivestreamComponent implements OnInit {
  ...
  constructor(
    ...
    //private streamComp : StreamComponent
  ) { 
  this.image_url = environment.image_url;
  this.socket_url = environment.socket_url;
  this.socket = io(this.socket_url);
    this.dataService.shareImgText.subscribe(response =>{
      if(response){
        this.image_arr = response; 
      }
    })
}
  ...
  ngOnInit() {
    if(localStorage.getItem("data")){
      ...
      this.user_type_ = data[0]['user_type'];
      //this.AttendeeName = this.uname +"#"+this.user_id+"#"+this.user_type;
      if(this.shopper_name) {

        this.AttendeeName = this.shopper_name +"#"+this.user_id+"#"+this.user_type;
      }else { 
          this.AttendeeName = this.uname +"#"+this.user_id+"#"+this.user_type;
      }
    ...
    this.getAllUser();
    // this.streamBroadcastData = this.streamComp.getBroadcastDetails();
    // console.log(this.streamBroadcastData,"++++++++++++88888")
    // alert(this.streamBroadcastData['broadcastStatus'])
    //this.getBroadcastDetails()
    
    // if(localStorage.getItem("user_id")){
    //   this.profile_Pic =localStorage.getItem("profile_Pic");
    //   this.user_id =localStorage.getItem("user_id");
    //   this.uname=localStorage.getItem("user_name");
    //   this.AttendeeName =localStorage.getItem("user_name")+"#"+this.user_id;
    // }
    ...
    if (this.isRecorder() || this.isBroadcaster()) {
      new AsyncScheduler().start(async () => {
        ...
        this.broadcastID= this.meeting.split('-').rethis.verse()[0];

        console.log('meeting-2-',this.meeting,this.broadcastID);
    ...
    getBroadcastDetails() {
      this.dataService.publicBroadcastingDetails(this.broadcastID,this.user_id).subscribe(response => {
        console.log(response,"streamBroadcastStatus");
        if(response['success'] == true) {
          this.streamBroadcastStatus = response['body']['broadcast_status']
        }
      });
    }
    ...
    // getBroadcastDetails() {
    //   let broad = 175
    //   this.dataService.publicBroadcastingDetails(broad,this.user_id).subscribe(response => {
    //     console.log(response,"broadcast Information")
    //      this.productLists = response['body']['productList']
    //      console.log(this.productLists,this.productLists.length)
    //   });
    // }

  getPurchaseAlert(checkOutData :any,userID:any, broadcastID:any){
    console.log(JSON.stringify(checkOutData, undefined, 2));
    this.checkout_data = checkOutData;
    // this.checkout_data['user_id'] = this.user_id;
    // this.checkout_data['broadcast_id'] = this.broadcastID
    let data = {
    ...
    this.socket.emit('checkoutTrigger',data);
    // const sendnewproductmessage= document.getElementById('sendnewproductmessage')
    //   console.log('before sendnewproductmessage---');
    //   console.log(sendnewproductmessage);
    //   if (sendnewproductmessage) {
    //     sendnewproductmessage.click();
    //   }
  ...
  liveSales(){
    ...
    this.socket.on('ShowFlashProduct',(response:any) => {
        ...
        // if(this.flesh_record.length > 0){
        //   this.get_variant(this.flesh_record.productList[0], 0);
        // } 
      });
  }
  ...
  getAllUser(){
    ...
    this.flesh_record = [];
      ...
      this.socket.emit('getLikesCount',{"broadcast_id":this.broadcastID,"user_id" : this.user_id});
    ...
    initParameters(): void {
      ...
      if (meeting) {
        ...
        (document.getElementById('inputName') as HTMLInputElement).focus();
         ...
         this.broadcastID= meeting.split('-').reverse()[0];
         this.liveSales();
         this.getTextonScreen();
         this.getLikesCount();
...
async initVoiceFocus(): Promise<void> {
 ...
 private async onVoiceFocusSettingChanged(): Promise<void> {
  ...
  var buttonTestsound=document.getElementById('button-test-sound');
    ...
    buttonSpeaker.addEventListener('click', _e => {
      new AsyncScheduler().start(async () => {
        if (this.toggleButton('button-speaker')) {
          try {
            this.setupSubscribeToAttendeeIdPresenceHandler("muted");
            await this.audioVideo.bindAudioElement(
              document.getElementById('meeting-audio') as HTMLAudioElement
            );
            buttonSpeaker.innerHTML= '<i class="fa fa-volume-up" style="padding:10px 15px"></i>';
          ...
          buttonSpeaker.innerHTML= '<i class="fa fa-volume-mute" style="padding:10px 15px"></i>';
    ...
    const textSendMessage = document.getElementById('sendjoinmessage') as HTMLDivElement;
    ...
    const shareBrodcast = document.getElementById('shareBrodcast') as HTMLButtonElement;
    shareBrodcast.addEventListener('click', e => {
          e.preventDefault();
          sendMessage("Just shared this show");
          textAreaSendMessage.rows = 1;
       
    });
    // const purchaseProduct = document.getElementById('sendnewproductmessage') as HTMLButtonElement;
    // purchaseProduct.addEventListener('click', e => {
    //   console.log(" purchase click----------------------------msg");
    //       e.preventDefault();
    //       sendMessage("Purchase the product");
    //       textAreaSendMessage.rows = 1;
    ...
    purchaseProduct.addEventListener('click', e => {
      e.preventDefault();
      if(this.image_arr.length > 0){
        this.image_arr.forEach((ele:any) =>{
    ...
    this.socket.on('confirnTrigger',(response:any) => {
      let txt = "";
      for(let i = 0 ; i < response['data'].length; i++){
        let data = response['data'][i];
        if(response['user_id'] == this.user_id){
          txt += "<p style='background-color: #28a745;color : #fff;font-weight: 600;padding: 2px 10px;border-radius: 2px;'>" + " Purchased " + data['quantity'] + ' ' + data['product_title'] + "\n" + '<img src="assets/images/confetti.png">' + '<img src="assets/images/giftbox.png"> </p>';
          //sendMessage("Purchased " + data['quantity'] + ' ' + data['product_title']);
          sendMessage(txt);
          textAreaSendMessage.rows = 1; 
        }        
      }
  ...
  async initializeMeetingSession(configuration: MeetingSessionConfiguration): Promise<void> {
    ...
    this.setupSubscribeToAttendeeIdPresenceHandler(0);
  ...
  updateRoster(): void {
    ...
    let innerVIEW =  (document.getElementById('viwer') as HTMLDivElement)
    if(innerVIEW) {
      innerVIEW.innerHTML ="Viewers <i class='fas fa-eye'></i> "+newRosterCount+"";
    }
    
    if (this.attendeeStart==1) {
      let innerView = (document.getElementById('roster') as HTMLDivElement);
      if(innerView){
        innerView.style.visibility = 'hidden';
      }
    }
    if(this.isJoinSent==0){
      this.isJoinSent=1;
      const clicksms= document.getElementById('sendjoinmessage')
      console.log('before sendjoinmessage---');
      if (clicksms) {
        clicksms.click();
      } 
  ...
  setupSubscribeToAttendeeIdPresenceHandler(ismuted:any): void {
    const handler = (
      attendeeId: string,
      present: boolean,
      ...
      this.audioVideo.realtimeSubscribeToVolumeIndicator(
        ...
        async (
          ...
          //console.log("Speaker-----",attendeeId,volume,muted,signalStrength,ismuted);
          if (!this.roster[attendeeId]) {
            return;
          }
          if (volume !== null) {
...
dataMessageHandler(dataMessage: DataMessage): void {
    ...
    if (!dataMessage.throttled) {
      ...
      console.log('ask--------',dataMessage.senderExternalUserId.split('#').slice(-3)[0])
...
*/
  ...
  videoTileDidUpdate(tileState: VideoTileState): void {
    ...
    if (tileIndex==1) {
      ...
      const toggleList = document.getElementById('tile-1');
      if(toggleList && toggleList.classList.contains('active')){
        this.fullVideo();
      }else{
        this.halfVideo(tileIndex);
      }
    }
    ...
    this.updateProperty(nameplateElement, 'innerText',this.streamTitle );//tileState!.boundExternalUserId!.split('#').slice(-1)[0]);
    }
  ...
  fullVideo(){
    const videoid = document.getElementById('tile-0') as HTMLDivElement;
    const videoidmini = document.getElementById('tile-1') as HTMLDivElement;
    videoid.classList.add('video-half');
    videoidmini.classList.add('video-half');
    videoidmini.classList.remove('full-video-fori');
    videoid.classList.remove('full-video-fori');
    
  }
  halfVideo(ids:any){
    const videoids = document.getElementById('tile-'+ids) as HTMLDivElement;                                                      
    const videoid = document.getElementById('tile-0') as HTMLDivElement;
    const videoidmini = document.getElementById('tile-1') as HTMLDivElement;
    videoids.classList.add('full-video-fori');
    //videoidmini.classList.add('video-width-small');
    videoidmini.classList.remove('video-half');
    videoid.classList.remove('video-half');
  }
  fullone(){
    const videoid = document.getElementById('tile-0') as HTMLDivElement;
    const videoidmini = document.getElementById('tile-1') as HTMLDivElement;
    videoid.classList.add('video-width');
    videoidmini.classList.add('video-width-small');
    videoidmini.classList.remove('video-width');
    videoid.classList.remove('video-width-small');
  }
  fulltwo(){
    const videoid = document.getElementById('tile-1') as HTMLDivElement;
    const videoidmini = document.getElementById('tile-0') as HTMLDivElement;
    videoid.classList.add('video-width');
    videoidmini.classList.add('video-width-small');
    videoid.classList.remove('video-width-small');
    videoidmini.classList.remove('video-width');
  ...
  getTextonScreen(){
    console.log('getTextonScreen -- ');
    this.socket.emit('setTextonScreen',{"broadcast_id":this.broadcastID,"text" :  this.streamTitle});
    this.socket.on('getTextonScreen',(res:any) => {
        console.log('getTextonScreen -- ',res);
        this.streamTitle = res;
    });

    this.socket.emit('getbroadcastMedia',{"broadcast_id":this.broadcastID});
    this.socket.on('ShowShardMedia',(response:any) => {
      console.log('ShowShardMedia --', response);
      this.testVideo = response;
      if(this.testVideo && this.testVideo!=''){
        //this.contentShareTypeChanged(this.ContentShareType.VideoFile, this.testVideo);
        // const buttonMute = document.getElementById('video-1') as HTMLVideoElement;
        // if (buttonMute) {
  ...
  getLikesCount(){                
    ...
    const islike = document.getElementById('live') as HTMLDivElement;
      if (res.islike==true) {
        islike.classList.add('active');
      }else{
        islike.classList.remove('active');        
      }
    });
  }
  ...
  changeVariant(event:any,variantName:any,product:any,index:any){
      ...
      console.log(this.variantValues[variantName]);
    ...
    get_variant(product:any, index: any){
      ...
      let variantCobination = res.join(" / ")
        ...
        var indexing =  this.flesh_record.productList[index].variants.findIndex(function(pro:any) {
              return pro['title'] == variantCobination
        })
      if(indexing != -1) {
        this.cart_variant = this.flesh_record.productList[index].variants[indexing]
        ...
        if(this.flesh_record.productList[index]['available_stock'] > 0){
    ...
    /*================ Add To cart ==================*/
    _addToCart(product:any, index: any) {
      this.get_variant(product, index)
      this.__saveCartData();
    ...
    this.cart = localStorage.getItem('cart')
      ...
      this.items = {
        ...
        price :  this.variant_Onprice[index],
        merchant_id : product['user_id'],
       broadcastID : this.streamBroadcastStatus
      }
      ...
      this.cart_variant = []
      this.variant_model = false;
  }

   addTocartModel(product:any,index:any) {
    this.model_options = product
     this.product_index = index
    setTimeout(() => {
      this.get_variant(product,index)
    }, 100);
     
     this.variant_model = true;
  }
  closeVariant(){
    this.variant_model = false;
  }
  ...
  getAvailableStock(){
		this.socket.on('updateAvailProductStock',(res:any) => {
			this.model_options.forEach((element:any) => {
