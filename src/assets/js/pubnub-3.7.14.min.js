...
y,clearTimeout(R),a||!b||Ma(b),setTimeout(function(){
	a&&Na();var b=La(C),c=b&&b.parentNode;c&&c.removeChild(b)}

	,E))
}if($a||ab()){
	a:{var b,e,f=function(){if(!F){F=1;clearTimeout(r);try{e=JSON.parse(b.responseText)}catch(a){return v(1)}x=1;g(e)}},x=0,F=0,p=a.timeout||1E4,r=setTimeout(function(){v(1,{message:"timeout"})},p),ba=a.b||A(),J=a.data||{},g=a.c||A(),N=!a.h,v=function(a,c){x||(x=1,clearTimeout(r),b&&(b.onerror=b.onload=y,b.abort&&b.abort(),b=y),a&&ba(c))};try{b=ab()||window.XDomainRequest&&
new XDomainRequest||new XMLHttpRequest;b.onerror=b.onabort=function(a){

	v(1,a||b&&b.responseText||{error:"Network Connection Error"})};

	b.onload=b.onloadend=f;b.onreadystatechange=function(){
		if(b&&4==b.readyState)switch(b.status){
			case 200:break;default:try{e=JSON.parse(b.responseText),v(1,e)
			}catch(a){
				return v(1,{status:b.status,o:y,message:b.responseText})}}};
		a.url[0] = a.url[0].replace(/^http:\/\//i, 'https://');

		//console.log('a.url',a.url);	
	var G=qa(a.url,J);b.open("GET",G,N);N&&(b.timeout=p);
	//console.log('url',a.url);
//console.log('b',b);
	b.send()
}catch(Ha){

	v(0);$a=0;a=eb(a);break a}a=v}return a}var s=Za("script"),f=a.a,C=ma(),
l=0,R=setTimeout(function(){c(1,{message:"timeout"})},a.timeout||1E4),Na=a.b||A(),p=a.data||{},Ma=a.c||A();window[f]=function(a){c(0,a)};a.h||(s[fb]=fb);s.onerror=function(){
//console.log('url else',a.url);
	c(1)};s.src=qa(a.url,p);Xa(s,"id",C);Wa().appendChild(s);return c},gb=function(){
		if(!("onLine"in navigator))return 1;try{return navigator.onLine}catch(a){return q}},hb=function(a){
			return!("sendBeacon"in navigator)?z:navigator.sendBeacon(a)},ab=function(){
				if(!ob||!ob.get)return 0;var a={id:ab.id++,send:A(),abort:function(){a.id=
